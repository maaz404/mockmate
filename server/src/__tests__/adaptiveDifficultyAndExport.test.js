...
describe('Adaptive difficulty PATCH + metrics export CSV', () => {
  ...
  const createRes = await request(app)
      ...
      .send({
        ...
        questions: [
          { text: 'Q1 explain event loop', category: 'technical', difficulty: 'intermediate' },
          { text: 'Q2 optimize React render', category: 'technical', difficulty: 'intermediate' },
          { text: 'Q3 CSS layout issue', category: 'technical', difficulty: 'intermediate' },
          { text: 'Q4 handle API errors', category: 'technical', difficulty: 'intermediate' },
          { text: 'Q5 state management tradeoffs', category: 'technical', difficulty: 'intermediate' }
        ]
  })
  .expect(CREATED);
    ...
    await request(app)
      ...
      .set('Authorization', 'Bearer test')
      .set('x-user-id', userId)
      .send({ answer: 'Sample answer', timeSpent: 42, facialMetrics: { eyeContact: 0.8 } })
      .expect(200);
  });
  ...
  it('exports metrics CSV including header row', async () => {
    const res = await request(app)
      ...
      .set('x-user-id', userId)
      .expect(200);
    expect(res.text.split('\n')[0]).toContain('questionIndex,category,difficulty,score');
  });
