...
class SessionSummaryService {
  ...
  calculateAggregateMetrics(interview) {
    ...
    const totalResponseTime = answeredQuestions.reduce(
  ...
  analyzeTimeMetrics(interview) {
    ...
    const fastestAnswer = answeredQuestions.find(
      (q) => q.timeSpent === fastestTime
    );
    const slowestAnswer = answeredQuestions.find(
      (q) => q.timeSpent === slowestTime
    );
  ...
  identifyPerformanceHighlights(interview) {
    ...
    const scoredQuestions = questions.filter(
      (q) => q.response && q.score?.overall !== undefined
    );
    ...
    const worstAnswers = sortedQuestions
      .slice(-3)
      .reverse()
      .map((q) => ({
        question: q.questionText.substring(0, 100) + "...",
    ...
    return {
      ...
      worstAnswers,
      improvementOpportunities:
        this.generateImprovementOpportunities(worstAnswers),
    };
  }
