...
class PDFGenerationService {
  ...
  addHeader(doc, summary, userProfile) {
    ...
    doc
      ...
      .fillColor("#6b7280")
      .text(
        `${summary.sessionInfo.jobRole} - ${summary.sessionInfo.experienceLevel}`,
        50,
        80
      );
    ...
    doc
      ...
      .text(
        `Candidate: ${userProfile?.firstName || "N/A"} ${
      ...
      .text(
        `Completed: ${new Date(
          summary.sessionInfo.completedAt
        ).toLocaleDateString()}`,
        300,
        125
      );
  ...
  addSessionOverview(doc, summary) {
    ...
    doc
      ...
      .text(
        `Questions Skipped: ${metrics.skippedQuestions || 0}`,
        50,
        startY + 30
      )
    ...
    // Right column
    doc
      .text(`Average Score: ${metrics.averageScore}/100`, 300, startY)
      .text(
        `Total Time: ${summary.sessionInfo.totalDuration} minutes`,
        300,
        startY + 15
      )
      .text(
  ...
  addCategoryScores(doc, summary) {
    ...
    categories.forEach((category, index) => {
      ...
      // Category name
      doc.fontSize(10).fillColor("#374151").text(category.category, 50, yPos);
      ...
      doc.fillColor(color).text(category.performance, 270, yPos);
    ...
    if (highlights.bestAnswers.length > 0) {
      ...
      highlights.bestAnswers.slice(0, 3).forEach((answer, index) => {
        ...
        doc
          ...
          .fillColor("#6b7280")
          .text(
            `Category: ${answer.category} | Time: ${answer.timeSpent}s`,
            50,
            yPos + 24
          );
      });
  ...
  addTimeAnalysis(doc, summary) {
    ...
    doc
      ...
      .text(
        `Average Response Time: ${timeAnalysis.averageTime} seconds`,
        50,
        startY + 15
      )
    ...
    if (timeAnalysis.fastestAnswer) {
      doc
        .text("Fastest Response:", 300, startY)
        .fontSize(9)
        .fillColor("#6b7280")
        .text(
          `${timeAnalysis.fastestAnswer.time}s - Score: ${timeAnalysis.fastestAnswer.score}/100`,
          300,
          startY + 12
        );
    }
    ...
    if (timeAnalysis.slowestAnswer) {
      doc
        ...
        .fillColor("#6b7280")
        .text(
          `${timeAnalysis.slowestAnswer.time}s - Score: ${timeAnalysis.slowestAnswer.score}/100`,
          300,
          startY + 42
        );
    }
  ...
  addOverallAssessment(doc, summary) {
    ...
    doc
      ...
      .text(
        `Readiness Level: ${assessment.readinessLevel.replace("-", " ")}`,
        50,
        startY + 30
      );
    ...
    const stars =
      "★".repeat(Math.floor(assessment.sessionRating)) +
      "☆".repeat(5 - Math.floor(assessment.sessionRating));
    doc
      .fontSize(16)
      .fillColor("#f59e0b")
      .text(
        `Session Rating: ${stars} (${assessment.sessionRating}/5)`,
        50,
        startY + 55
      );
    ...
    doc
      ...
      .fontSize(10)
      .fillColor("#6b7280")
      .text(assessment.recommendation, 50, startY + 100, {
        width: 500,
        align: "left",
      });
  ...
  addFooter(doc, summary) {
    ...
    doc
      ...
      .fillColor("#9ca3af")
      .text(
        `Generated by MockMate on ${new Date(
          summary.generatedAt
        ).toLocaleDateString()}`,
        50,
        pageBottom - 20
  ...
  addSectionTitle(doc, title) {
    doc.fontSize(14).fillColor("#1f2937").text(title, 50, doc.y).fontSize(10);
  ...
  drawPerformanceBar(doc, x, y, width, height, value, total, color, label) {
    ...
    // Background bar
    doc.rect(x, y, width, height).fillColor("#f3f4f6").fill();

    // Fill bar
    if (fillWidth > 0) {
      doc.rect(x, y, fillWidth, height).fillColor(color).fill();
    }

    // Label
    doc.fillColor("#374151").text(label, x + width + 10, y - 2);

    // Count
    doc.fillColor("#6b7280").text(`${value}`, x + width + 120, y - 2);
  }
