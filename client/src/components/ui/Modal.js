  ...
  return (
    ...
    >
      ...
      <div
        ...
        } mx-auto bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-2xl shadow-lg p-6 focus:outline-none`}
      >
        <div className="flex items-start justify-between mb-4">
          <h2 className="text-lg font-semibold text-surface-900 dark:text-surface-50 pr-6">
            {title}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="text-sm text-surface-700 dark:text-surface-300 space-y-4">
          {children}
        </div>
        {footer !== undefined ? (
          <div className="mt-6 pt-4 flex justify-end gap-2 border-t border-surface-200 dark:border-surface-700">
            {footer}
          </div>
        ) : null}
      </div>
