...
const StreakStrip = ({ days }) => {
  if (!days || !days.length)
    return (
      <div className="bg-surface-800/50 backdrop-blur-sm rounded-xl border border-surface-700 p-5 text-sm text-surface-400">
        No activity yet.
      </div>
    );
  ...
  return (
    <div className="bg-surface-800/50 backdrop-blur-sm rounded-xl border border-surface-700 p-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-surface-400">Consistency</p>
          <h3 className="text-lg font-semibold text-white">21‑Day Activity</h3>
        </div>
        <div className="text-xs text-surface-400">{pct}% active</div>
      </div>
      <div className="grid grid-cols-21 gap-1" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(0,1fr))` }}>
        {days.map((d) => (
          <div
            key={d.date}
            role="listitem"
      ...
      <p className="mt-3 text-[11px] text-surface-400">
        Aim for steady daily touches—small consistent practice compounds faster than bursts.
      </p>
    </div>
  );
};
