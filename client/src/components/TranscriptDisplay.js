...
const TranscriptDisplay = ({ interviewId, questionIndex, className = "" }) => {
  ...
  useEffect(() => {
    const fetchTranscription = async () => {
      try {
        ...
        const response = await videoService.getTranscription(
          interviewId,
          questionIndex
        );
  ...
  if (loading) {
    return (
      <div
        className={`bg-surface-50 border border-surface-200 rounded-lg p-4 ${className}`}
      >
        <div className="flex items-center justify-center space-x-2">
          <svg
            className="w-5 h-5 text-surface-400 animate-spin"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <span className="text-surface-600">Loading transcription...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={`bg-white border border-surface-200 rounded-lg ${className}`}
    >
      <div className="px-4 py-3 border-b border-surface-200 bg-surface-50 rounded-t-lg">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-medium text-surface-900 flex items-center space-x-2">
            <svg
        ...
        {transcription?.status === "completed" && transcription?.text ? (
          <div className="space-y-3">
            ...
            {transcription.generatedAt && (
              <div className="text-xs text-surface-500 pt-2 border-t border-surface-100">
                Transcribed on{" "}
