 ...
 */
export function FacialMetricsPanel({
  metrics,
  analyzing,
  onStop,
  onStart,
    ...
    <div className="mt-6 p-4 rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/60 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold tracking-wide text-surface-700 dark:text-surface-300 flex items-center gap-2">
          ...
          {!analyzing && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-surface-200 dark:bg-surface-700 text-surface-600 dark:text-surface-400 border border-surface-300 dark:border-surface-600">
              idle
            </span>
          )}
          {analyzing && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-600 text-white border border-emerald-500 animate-pulse">
              live
            </span>
          )}
        </h3>
        <div className="flex gap-2">
          {!initialized && (
            <button onClick={onStart} className="btn-ghost !px-2 !py-1 text-xs">
              Enable
            </button>
          )}
      ...
      {initialized ? (
          ...
          {[
            ...
            <div key={label} className="space-y-1">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-surface-900 dark:text-surface-200 tabular-nums">
        ...
        <p className="text-xs text-surface-500 dark:text-surface-400">
          Enable to receive real-time speaking feedback. Processing happens
          locally (mock mode).
        </p>
      )}
    </div>
