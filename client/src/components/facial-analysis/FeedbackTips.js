...
const FeedbackTips = ({
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      default:
        return "bg-surface-50 border-surface-200 text-surface-800";
    }
  };
      ...
      <AnimatePresence>
        {showTip && (
          <motion.div
            ...
            className={`max-w-sm rounded-lg border p-4 shadow-lg ${getPriorityColor(
              currentTip.priority
            )}`}
          >
            <div className="flex items-start space-x-3">
              <div className="flex-shrink-0">{getTipIcon(currentTip.type)}</div>
