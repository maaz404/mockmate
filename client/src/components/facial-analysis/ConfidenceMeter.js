  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-4 ${className}`}>
      ...
      <div className="text-center mb-4">
        <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full ${getScoreColor(metrics.confidenceScore)} mb-2`}>
          <span className="text-2xl font-bold">{Math.round(metrics.confidenceScore)}</span>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-700">Blink Rate</span>
          </div>
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-900 w-8">
            ...
            <div className={`w-2 h-2 rounded-full ${
              metrics.blinkRate >= 15 && metrics.blinkRate <= 25 ? 'bg-green-500' : 'bg-yellow-500'
            }`}></div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-700">Expressions</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-16 bg-gray-200 rounded-full h-2">
              <motion.div 
                ...
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(100, metrics.smilePercentage)}%` }}
                transition={{ duration: 0.5 }}
              />
            </div>
            <span className="text-sm font-medium text-gray-900 w-8">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-16 bg-gray-200 rounded-full h-2">
              <motion.div 
                ...
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(100, 100 - metrics.offScreenPercentage)}%` }}
                transition={{ duration: 0.5 }}
              />
            </div>
            <span className="text-sm font-medium text-gray-900 w-8">
